...
export default function Equipo() {
  ...
  const getRoleIcon = (rol: string) => {
    ...
    if (r.includes('programador') || r.includes('dev')) return <Code className="h-4 w-4" />;
    if (r.includes('diseñador') || r.includes('design')) return <Target className="h-4 w-4" />;
    if (r.includes('artist') || r.includes('arte')) return <Palette className="h-4 w-4" />;
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {miembros.map((miembro) => (
              <Card key={miembro.id} className="border-border overflow-hidden relative">
                {miembro.enLinea && (
                    ...
                    <div className="flex gap-4 items-center">
                      <div className="relative">
                        <Avatar className="h-14 w-14 border border-border">
                          ...
                          <AvatarFallback className="bg-muted text-lg">{miembro.nombre.charAt(0)}</AvatarFallback>
                        </Avatar>
                        {miembro.enLinea ? (
