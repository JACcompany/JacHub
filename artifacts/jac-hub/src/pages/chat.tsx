...
import { Send, Wifi, WifiOff, Hash, Users, Crown } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { es } from "date-fns/locale";

const SALAS = [
  { id: "general", nombre: "general", descripcion: "Chat del equipo" },
  { id: "desarrollo", nombre: "desarrollo", descripcion: "Programación y engine" },
  { id: "arte", nombre: "arte", descripcion: "Assets y diseño visual" },
  { id: "qa", nombre: "qa", descripcion: "Testing y bugs" },
  { id: "anuncios", nombre: "anuncios", descripcion: "Solo OWNER" },
];
  ...
  const cargarHistorial = useCallback(async (sala: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMensajes(data.map((m: any) => ({ ...m, tipo: "mensaje" })));
        scrollToBottom();
      }
    } catch { /* ignore */ }
          ...
          <div className="h-12 flex items-center justify-between px-4 border-b border-border/50 bg-card/30 flex-shrink-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground hidden md:block">
                — {SALAS.find(s => s.id === salaActual)?.descripcion}
              </span>
            </div>
            <div className="flex items-center gap-1.5">
              {conectado
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-2">
            {cargando ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-muted-foreground/60 text-xs mt-1">¡Sé el primero en escribir algo!</p>
              </div>
            ) : (
              mensajes.map((msg, i) => {
                if (msg.tipo === "sistema") {
                ...
                const esOwner = isOwnerMsg(msg.autorEmail);
                const esMio = msg.autorEmail === user?.email;
                const esConsecutivo = i > 0 && mensajes[i-1].tipo === "mensaje" && mensajes[i-1].autorEmail === msg.autorEmail;
                  ...
                  <div key={msg.id ?? i} className={`flex gap-3 group ${esMio ? "flex-row-reverse" : "flex-row"}`}>
                    {!esConsecutivo && (
                      <div className={`w-8 h-8 rounded-lg flex-shrink-0 flex items-center justify-center text-sm font-bold
                        ${esOwner ? 'bg-amber-900/40 border border-amber-400/50 text-amber-300' : 'bg-primary/10 border border-primary/20 text-primary'}`}>
          ...
          <div className="p-3 border-t border-border/50 bg-card/30 flex-shrink-0">
            <form onSubmit={enviar} className="flex gap-2">
              <Input
                ...
                placeholder={`Mensaje en #${salaActual}...`}
                disabled={!conectado}
                maxLength={2000}
              ...
              <Button
                ...
                size="icon"
                className="shadow-[0_0_8px_rgba(0,255,136,0.2)] flex-shrink-0"
              >
            ...
            <div className="flex items-center gap-1 mt-1.5">
              ...
              <span className="text-[10px] text-muted-foreground/50">
                Escribe con moderación · Los mensajes son visibles para todo el equipo
              </span>
            </div>
          </div>
        </div>
